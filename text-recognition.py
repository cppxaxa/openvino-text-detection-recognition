...
def main():
    ...
    # print('Nodes', [op.get_type_name() for op in ops if op.friendly_name in net.outputs])
    # ops = func.get_ordered_ops()
    # -----------------------------------------------------------------------------------------------------
    ...
    images_hw = []
    for i in range(n):
        image = cv2.imread(args.input[i], 0)
        ih, iw = image.shape
    ...
    global textRecognizer
    result = textRecognizer.ctc_decoder(res['shadow/LSTMLayers/transpose_time_major'])
    print(result)
